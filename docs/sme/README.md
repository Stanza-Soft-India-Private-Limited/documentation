...
| [SME_OFFERS_API.md](./SME_OFFERS_API.md) | Promotional offer campaigns — create/schedule/kill discounted pricing without an app release; the two gates (`status=LIVE` **and** date window), the one-live-campaign rule, the immutable public `code` used in `/open/offer/<CODE>` share links, per-plan `bonusDays`, and presigned S3 hero-image upload. **Prices are display strings today — a campaign does not yet change what is charged** |
